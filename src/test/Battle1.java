...
//					new ComputerBattle(f,s.v.get(s.num3[a]).trim(),s.v1.get(s.num2[b]).trim(),s.NewMap.get(4));
				...
				if (s.team1wincount == 3) {
					System.out.println("승리팀:" + s.NewTeam.get(s.num[2 * j - 1]) + " 스코어 " + s.team1wincount + ":"
							+ s.team2wincount);
					...
					System.out.println(
							"승리팀:" + s.NewTeam.get(s.num[2 * j]) + " 스코어 " + s.team1wincount + ":" + s.team2wincount);

...
//			ResultSet srs = stmt.executeQuery("SELECT * FROM sunsu_data"); // 테이블의 모든 데이터 검색
	...
	public void array() {
		try {
			...
			ResultSet srs = stmt.executeQuery("select * from sunsu_data order by 승 desc");// 테이블의 특정 선수 데이터 검색
			int a = 0;
			sunsuname = new ArrayList<String>();
			winarray = new ArrayList<Integer>();
			losearray = new ArrayList<Integer>();
			while(srs.next()&& a<10) {
				sunsuname.add(srs.getString("이름"));
				winarray.add(Integer.parseInt(srs.getString("승")));
				losearray.add(Integer.parseInt(srs.getString("패")));
	...
	public ArrayList<String> getSunsuname() {
		return sunsuname;
	}

	public void setSunsuname(ArrayList<String> sunsuname) {
		this.sunsuname = sunsuname;
	}

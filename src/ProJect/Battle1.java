...
////////////////////12강 토너먼트 시뮬레이션///////////////////////////////////////////////
	...
	String sunsu2;
	String map;
	Battle1(JFrame f, String Myteam) {
		...
		for (int j = 1; j < 6; j++) {
			...
			try {
				...
				BufferedReader br = new BufferedReader(
						new FileReader("src/" + s.NewTeam.get(s.num[2*j-1]) + ".txt")); // 불러올
																								// 파일이름
				s.v.clear();
			...
			try {
				String str;
				BufferedReader br = new BufferedReader(
						new FileReader("src/" + s.NewTeam.get(s.num[2*j]) + ".txt")); // 불러올
																									// 파일이름
				s.v1.clear();
				while ((str = br.readLine()) != null) {
			...
			for (int i = 0; i < 4; i++) {
				...
				if(s.team1wincount == 2 && s.team2wincount ==2) {
					...
					if(winrate2<=winrate) {
						...
						s.Lose = true;
						s.WIN = false;
						s.sunsudb.record(s.v.get(a), s.WIN, s.type2);
						s.sunsudb.record(s.v1.get(b), s.Lose, s.type1);
...
//					new ComputerBattle(f,s.v.get(s.num3[a]).trim(),s.v1.get(s.num2[b]).trim(),s.NewMap.get(4));
				...
				if(s.team1wincount == 3) {
					System.out.println("승리팀:"+ s.NewTeam.get(s.num[2*j-1]) + " 스코어 " + s.team1wincount +":" + s.team2wincount);
					s.NewTeam.add(s.NewTeam.get(s.num[2*j-1]));
					break;
				}
				else if(s.team2wincount == 3) {
					System.out.println("승리팀:"+ s.NewTeam.get(s.num[2*j])+ " 스코어 " + s.team1wincount +":" + s.team2wincount);
					s.NewTeam.add(s.NewTeam.get(s.num[2*j]));
					break;
				}
			}
